...
export function createAccountRouter(deps: Dependencies): Router {
  ...
  const router = Router();
  const uploadTaxDocument = createUploadMiddleware("document", deps.maxUploadBytes);
  ...
  router.post("/account/email", async (req, res) => {
    ...
    if (!currentPassword || !await verifyPassword(currentPassword, current.user.password_hash)) {
      res
        .status(403)
        .type("html")
        .send(
