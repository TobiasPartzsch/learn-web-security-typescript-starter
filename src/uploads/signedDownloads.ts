...
export function verifySignedDownload(
  ...
  const hexBuffer: Buffer = Buffer.from(signature, "hex");
  const expectedSignature: Buffer = Buffer.from(signDownload(signingKey, fileId, expires), "hex")
  return timingSafeEqual(expectedSignature, hexBuffer)
}
